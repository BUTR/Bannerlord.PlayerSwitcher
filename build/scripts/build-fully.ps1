...
$proj = [IO.Path]::GetFullPath((Join-Path -Path $BasePath -ChildPath 'src/PlayerSwitcher/PlayerSwitcher.csproj')); # Normalize path
$bin =  [IO.Path]::GetFullPath((Join-Path -Path $OutputPath -ChildPath 'Modules/PlayerSwitcher/bin/Win64_Shipping_Client')); # Normalize path
$pdll = Join-Path -Path $bin -ChildPath 'PlayerSwitcher*.dll';
$ppdb = Join-Path -Path $bin -ChildPath 'PlayerSwitcher*.pdb';
